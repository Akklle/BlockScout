...
export const TransactionItem = (props: wrapperTransaction) => {
    ...
    return (<tr className={styles.tableRow}>
        <td className={styles.tdCell}>
            <div>
                <NavLink className={classNames(styles.address, styles.fontWeight500)} to={"/transaction/" + currentTransaction.hash}>{stringTruncateFromLeft(currentTransaction.hash, 28)}</NavLink>
                <p className={styles.hashTime}>{getTimeFromTimestamp(currentTransaction.timestamp)}</p>
            </div>
        </td>
        <td className={styles.tdCell}>
        ...
        <td className={styles.tdCell}><p className={styles.method}>{currentTransaction.method}</p>
        </td>
        <td className={styles.tdCell}>
            <NavLink className={classNames(styles.address, styles.fontWeight500)} to={"/block/" + currentTransaction.block}>{currentTransaction.block}</NavLink>
        </td>
        <td className={styles.tdCell}>
        ...
        <td className={styles.tdCellRight} align={"right"}>{currentTransaction.value === "0" ? 0 : round((Number(currentTransaction.fee?.value) / 10 ** 18), 5)}</td>
        <td className={styles.tdCellRight} align={"right"}>{round((Number(currentTransaction.fee?.value) / 10 ** 18), 5)}</td>
